...
def init_redis_client():
    ...
    try:
        logger.info(f"尝试连接到 Redis: {redis_host}:{app_configs.get('REDIS_PORT')}:{app_configs.get('REDIS_PASSWORD')}:{app_configs.get('REDIS_SSL_ENABLED')}:{app_configs.get('REDIS_DB')}")
